...
function addToAllItems($i, $item)
{
    ...
    [void]$global:indexesThatGotStuckDuringCleanupOutlookEmails.Remove($i);
}
...
foreach ($item in $global:Inbox.Items)
{
    ...
    if (!($global:indexesThatGotStuckDuringCleanupOutlookEmails.Contains($i)))
    {
        [void]$global:indexesThatGotStuckDuringCleanupOutlookEmails.Add($i);
        get-job -Name 'GetReceivedTime' -ErrorAction Ignore | remove-job;
        ...
        {
            cleanupOutlook;
            throw 'Outlook not running, possibly you have killed it in order to run the script again (?)';
        }
        ...
        else
        {
            ...
            {
                Write-Host "Not considering $i [$($item.Subject)] because it failed with $($_.Exception.Message)" -ForegroundColor Magenta;
            }
        }
    }
    else
    {
        Write-Host "Avoiding item at index $i since it failed in the last run..." -ForegroundColor Magenta;
    }
