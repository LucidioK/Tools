...
{
    if ($null -eq $global:AllDCs)
    {
        Write-Host "Getting DNS host... " -ForegroundColor Green -NoNewline;
        $global:CurrentDomain = (Get-ADDomain).DnsRoot;
        Write-Host $global:CurrentDomain -ForegroundColor Green;
        Write-Host "Getting other DNS hosts... " -ForegroundColor Green -NoNewline;
        $global:AllDCs = (Get-ADForest).Domains | 
                    ForEach-Object{ (Get-ADDomainController -Filter * -Server $_).Domain } | 
                    Select-Object -Unique | 
                    Where-Object { $_ -ne $global:CurrentDomain } | 
                    Sort-Object;
        $global:AllDCs = @($global:CurrentDomain) + $global:AllDCs;
        Write-Host "$($global:AllDCs.Count) DNS hosts found. " -ForegroundColor Green;
    }
    
    Write-Host "Retrieving user $id" -ForegroundColor Green;
    foreach ($dc in $global:AllDCs)
    {
        Write-Host " Trying to get user on $dc" -ForegroundColor Green;
        $user = $null;
        try {
            $user = Get-AdUser -Identity $id -Properties Manager,Created,Title, Department, extensionAttribute5 -Server $dc -ErrorAction SilentlyContinue;
        }
